...
export function createTimeline(canvas, dotNetRef, options) {
    return new BlazorResourceTimeline(canvas, dotNetRef, options);
}

class BlazorResourceTimeline {
    constructor(canvas, dotNetRef, options) {
        // The canvas element is passed directly from Blazor as an
        // ElementReference, so no getElementById lookup is needed.
        this.canvas = canvas;
        ...
        this.config = {
            ...
            // renderer fetches a window buffered by windowBufferFactor viewports
            ...
            // When the host supplies an HTML resource-column template, the canvas
            // stops drawing the resource labels/chevrons (an HTML overlay renders
            // them instead) and the renderer reports its visible rows to .NET.
        ...
        // pointer up. See handlePointerDown/Move/Up and drawEditGhost.
        this.edit = null;
        ...
        this._onResize = () => {
            // Browsers without device-pixel-content-box (Safari) don't notify
            // on DPR-only changes; drop the cached device size so resizeCanvas
            // recomputes it from the current devicePixelRatio.
        ...
        this._onPointerLeave = () => { this._hideTooltip(); this.canvas.style.cursor = ''; };
        ...
        // announced here so screen-reader users get feedback from the canvas
    ...
    _setupEventListeners() {
        ...
        // the pointer on the canvas, so move/up keep tracking even when it
        // leaves the canvas and the drag completes on release anywhere.
        ...
        // "application"), not the canvas, which cannot take focus itself.
    ...
    // Observes the wrapper (viewport size in CSS pixels) and the canvas
    // (backing-store size in device pixels, via device-pixel-content-box where
    // supported) so both container resizes and devicePixelRatio changes are
    // picked up. This also covers the initial layout: the observer fires once
    // as soon as each target has a size.
    _setupResizeObserver() {
        this._resizeObserver = new ResizeObserver((entries) => this._handleResizeEntries(entries));
        try {
            this._resizeObserver.observe(this.canvas, { box: 'device-pixel-content-box' });
            this._observesDevicePixels = true;
        } catch {
            // Safari does not support device-pixel-content-box; fall back to
            // CSS pixels multiplied by devicePixelRatio.
    ...
    _handleResizeEntries(entries) {
        for (const entry of entries) {
            if (entry.target === this.wrapper) {
            ...
            } else if (entry.target === this.canvas) {
                ...
                if (dpc) {
                    ...
                    this._deviceW = Math.round(w * dpr);
                    this._deviceH = Math.round(h * dpr);
                }
            }
        }
        this.resizeCanvas();
    ...
    // scrollbar onto the larger virtual range (see resizeCanvas / _handleScroll)
    ...
    resizeCanvas() {
        ...
        // The canvas is sticky-positioned over the viewport; its CSS size is
        // the viewport size while the backing store is sized in device pixels
        // for crisp output on HiDPI screens. The scale is capped so very high
        // DPR displays (some phones report 3-4) don't multiply the fill cost.
        const maxScale = 2;
        let deviceW, deviceH;
        if (this._deviceW > 0 && this._deviceH > 0 && this._deviceW <= cssW * maxScale) {
        ...
        const spacerWidth = Math.min(virtualWidth, BlazorResourceTimeline.MAX_SPACER_PX);
    ...
    render() {
        if (!this._hasTimeRange() || this.canvas.width === 0 || this.canvas.height === 0) {
    ...
    // Performs one full canvas paint. Shared by render() and the scroll
    // handler so scroll-triggered updates don't incur a second rAF.
    _paintFrame() {
        // Keep the HTML resource overlay aligned (covers keyboard/programmatic
        // scroll that sets scrollY without a scroll event).
        this._syncResourceOverlay();
        try {
            // Clear in device pixels, then draw in CSS pixels: the
            // transform maps CSS-pixel drawing onto the (possibly
            // higher-resolution) device-pixel backing store.
            this.ctx.setTransform(1, 0, 0, 1, 0, 0);
            this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
            this.ctx.setTransform(this._scaleX, 0, 0, this._scaleY, 0, 0);
    ...
    drawBackground() {
        ...
        const contentWidth = this._viewportW - c.resourceAxisWidth;
        const contentHeight = this._viewportH - c.timeAxisHeight;
        ...
        this.ctx.fillRect(0, 0, c.resourceAxisWidth, c.timeAxisHeight);
    ...
    // Bottom row: hourly ticks and hour-of-day labels (thinned to fit the zoom).
    drawHourRow(startX, visibleEndX, rowTop, axisHeight) {
        ...
        this._forEachZonedHour(this.visibleTimeRange.start, this.visibleTimeRange.end, (currentTime) => {
            ...
            if (x >= startX && x <= visibleEndX) {
                ...
                ctx.fillText(hour.toString().padStart(2, '0'), x, labelY);
            }
        });
    }
    ...
    drawResourceAxis() {
        ...
        // When an HTML resource-column template is active, the overlay draws the
        // labels/chevrons; the canvas only paints the axis background/border.
        ...
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.rect(0, startY, axisWidth - 1, visibleEndY - startY);
        this.ctx.clip();
        ...
        for (let i = visibleStart; i < visibleEnd; i++) {
            ...
            const midY = y + c.resourceHeight / 2;
            // Depth reserves room on the left; group rows get a chevron there.
            const leftPad = 8 + row.depth * indent;

            if (row.hasChildren) {
                // Chevron: ▸ collapsed, ▾ expanded. Drawn left-aligned at the
                // row's indent; clicking this band toggles the group (see
                // handlePointerDown / _rowAtY / _toggleGroup).
                const collapsed = this._collapsed.has(row.resource.id);
                this.ctx.fillStyle = c.colors.label;
                this.ctx.textAlign = 'left';
                this.ctx.font = '10px sans-serif';
                this.ctx.fillText(collapsed ? '\u25B6' : '\u25BC', leftPad, midY);

                this.ctx.fillStyle = c.colors.dateLabel;
                this.ctx.font = 'bold 13px sans-serif';
                this.ctx.fillText(row.resource.name, leftPad + 14, midY);
            } else {
                this.ctx.fillStyle = c.colors.label;
                this.ctx.textAlign = 'left';
                this.ctx.font = '13px sans-serif';
                this.ctx.fillText(row.resource.name, leftPad, midY);
            }
        }
    ...
    drawAllocationBars() {
        ...
        for (let resourceIndex = firstResource; resourceIndex < lastResource; resourceIndex++) {
            ...
            for (let i = firstIndex; i < resourceAllocations.length; i++) {
                ...
                // selection outline), drawn only while the component holds
                // focus so it never lingers after the user clicks away.
                if (this._hasFocus && this._focusAlloc && this._focusAlloc.id === alloc.id) {
                    const ringLeft = drawStartX - 2;
                    const ringWidth = Math.max(barWidth, drawEndX - drawStartX) + 4;
                    this.ctx.save();
                    this.ctx.strokeStyle = c.colors.focus;
                    this.ctx.lineWidth = 2;
                    this.ctx.setLineDash([3, 2]);
                    this.ctx.strokeRect(ringLeft, barTop - 2, ringWidth, barHeight + 4);
                    this.ctx.restore();
                }

                // Per-bar labels and icons (only when present, to keep the
                // common path cheap). Start/end decorations sit outside the
                // full span (edge bars included). Level-of-detail: skip
                // decorations for bars too narrow to sit beside without
                // overlapping their neighbours, which also avoids a flood of
                // unreadable, overlapping text on dense timelines.
    ...
    handlePointerDown(e) {
        ...
        // Route this pointer's subsequent move/up to the canvas even if it
    ...
    // Returns the nearest bar under the given canvas point and its resource row
    ...
    // Draws the semi-transparent preview of the allocation being edited at its
    // proposed position/size (and target row), on top of the committed scene.
    drawEditGhost() {
        const ed = this.edit;
        ...
        const width = Math.max(c.minBarWidth, x2 - x1);

        const ctx = this.ctx;
        ctx.save();
        // Clip to the content area so the ghost never spills onto the axes.
        ctx.beginPath();
        ctx.rect(
            c.resourceAxisWidth,
            c.timeAxisHeight,
            this._viewportW - c.resourceAxisWidth,
            this._viewportH - c.timeAxisHeight
        );
        ctx.clip();

        ctx.globalAlpha = 0.7;
        ctx.fillStyle = ed.alloc.color || c.colors.barSelected;
        ctx.fillRect(x1, barTop, width, barHeight);
        ctx.globalAlpha = 1;
        ctx.strokeStyle = c.colors.barSelectedBorder;
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 2]);
        ctx.strokeRect(x1 + 0.5, barTop - 0.5, width, barHeight + 1);
        ctx.setLineDash([]);

        // Time readout above the ghost for precise feedback while dragging.
        if (this._announceDtf) {
            const label = `${this._announceDtf.format(ed.previewStart)} – ${this._announceDtf.format(ed.previewEnd)}`;
            ctx.font = c.barLabelFont;
            ctx.fillStyle = c.colors.dateLabel;
            ctx.textBaseline = 'bottom';
            ctx.fillText(label, x1, barTop - 2);
        }
        ctx.restore();
    }
    ...
    // Enables the HTML resource-column overlay: the canvas stops drawing labels,
    // the overlay element is translated to follow vertical scroll, and the
    // current rows are reported to .NET so it can render the templates.
    ...
    // Visible-row index at a canvas y (content or resource-axis band), or -1.
    _rowAtY(canvasY) {
        const c = this.config;
    ...
    // Applies new visual options at runtime (e.g. theme or time-zone change).
    // Dimensions may change, so the canvas is re-laid out and repainted.
    setOptions(options) {
        this._applyOptions(options);
        this._rebuildDateFormatters();
    ...
    // canvas x (or the viewport center) fixed. Returns the new scale.
    ...
    // Puts the renderer into windowed mode: resources and overall range are set,
    ...
    // The time window the renderer wants loaded for the current viewport: the
